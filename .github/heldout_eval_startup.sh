...
echo "=== arch heldout-eval boot $(date -u) — task=crystallize-no-cook pr=${PR_NUMBER:-?} ==="
...
if [ -d "crystallize-no-cook" ]; then cd "crystallize-no-cook"; echo "cd into task subdir: $(pwd)"; fi
...
PUBLIC_BULLETS=""
PUBLIC_BULLETS+=$'\n'"- test_acc: \`$(jq -r '.metrics["test_acc"] // "n/a"' "$OUT")\`"
PUBLIC_BULLETS+=$'\n'"- decisiveness: \`$(jq -r '.metrics["decisiveness"] // "n/a"' "$OUT")\`"
PUBLIC_BULLETS+=$'\n'"- decisiveness_retention: \`$(jq -r '.metrics["decisiveness_retention"] // "n/a"' "$OUT")\`"
PUBLIC_BULLETS+=$'\n'"- composable_acc: \`$(jq -r '.metrics["composable_acc"] // "n/a"' "$OUT")\`"
PUBLIC_BULLETS+=$'\n'"- noncomposable_acc: \`$(jq -r '.metrics["noncomposable_acc"] // "n/a"' "$OUT")\`"
PUBLIC_BULLETS+=$'\n'"- train_acc: \`$(jq -r '.metrics["train_acc"] // "n/a"' "$OUT")\`"
...
if [ "$EVAL_EXIT" -eq 0 ] && [ "$SCORE" != "null" ]; then
  ...
  echo "=== heldout-eval failed (exit=$EVAL_EXIT, score=$SCORE); keeping container alive for SSH debug. ==="
