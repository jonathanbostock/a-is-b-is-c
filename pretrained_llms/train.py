...
class TrainingConfig:
    ...
    freeze_mlp_top_frac: float = 0.0  # freeze MLP in the TOP fraction of transformer layers (those nearest the output/residual the decisiveness head reads); train only the lower/middle-layer MLPs where forward-transitive composition forms
...
def run_single_repeat_training(
    *,
    ...
    if config.use_lora:
        ...
        if config.freeze_mlp_top_frac > 0.0:
            # Freeze the MLP of the TOP fraction of transformer layers. Those
            # layers write most directly into the final residual stream that the
            # forced-choice/decisiveness head reads, so freezing them protects
            # decisiveness further; lower/middle-layer MLPs (where the
            # forward-transitive composition forms) stay trainable. Aim: shift the
            # crystallization<->retention frontier out vs freezing attention alone.
            n_total_layers = int(model.config.num_hidden_layers)
            first_frozen = int(round((1.0 - config.freeze_mlp_top_frac) * n_total_layers))
            n_mlp_frozen = 0
            import re as _re
            for name, p in model.named_parameters():
                if ".mlp." not in name or not p.requires_grad:
                    continue
                m = _re.search(r"layers\.(\d+)\.", name)
                if m is not None and int(m.group(1)) >= first_frozen:
                    p.requires_grad_(False); n_mlp_frozen += p.numel()
            print(f"[freeze_mlp_top_frac] froze MLP in layers >= {first_frozen}/{n_total_layers} ({n_mlp_frozen/1e6:.1f}M params)")
