...
class TrainingConfig:
    ...
    freeze_mlp_down: bool = False  # freeze MLP down_proj (the residual-writer); train only gate/up_proj — keeps the MLP->residual projection geometry fixed so the decisiveness readout is less disturbed while gate/up still crystallize
...
def run_single_repeat_training(
    *,
    ...
    if config.use_lora:
        ...
        if config.freeze_mlp_down:
            # Freeze MLP down_proj (the residual-writer) and train only gate/up.
            # The composition still forms (gate/up change the internal activations)
            # but the fixed down_proj keeps the MLP's write direction into the
            # residual stream unchanged, so the forced-choice readout is less
            # disturbed — a candidate frontier-shift for retention.
            n_down = 0
            for name, p in model.named_parameters():
                if "mlp.down_proj" in name and p.requires_grad:
                    p.requires_grad_(False); n_down += p.numel()
            print(f"[freeze_mlp_down] froze {n_down/1e6:.1f}M down_proj params")
